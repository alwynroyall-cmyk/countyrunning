...
def _write_league_narrative(doc: Document,
    ...
    from league_scorer.process.rules import get_season_final_race
...
def _build_race_header(doc: Document, race_name: str, race_num: int,
    ...
    waa_path = (images_dir / "WRRL_logo-629x400.png") if images_dir else None
    if waa_path and waa_path.exists():
        p = waa_cell.paragraphs[0]
        p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        p.add_run().add_picture(str(waa_path), width=Cm(2.2))
...
def _build_cover_header(
    ...
    """Three-column table: shield left, title centre, WAA logo right, all on navy."""
    tbl = doc.add_table(rows=1, cols=3)
    logo_cell  = tbl.rows[0].cells[0]
    title_cell = tbl.rows[0].cells[1]
    waa_cell   = tbl.rows[0].cells[2]
    ...
    # WAA logo at the top right
    waa_path = (images_dir / "WRRL_logo-629x400.png") if images_dir else None
    if waa_path and waa_path.exists():
        p = waa_cell.paragraphs[0]
        p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        run = p.add_run()
        run.add_picture(str(waa_path), width=Cm(3.0))
    ...
    r = p1.add_run(f"WRRL Season {year}")
    r.bold = True
    r.font.size = Pt(22)
    ...
    subtitle = (
        "Season Summary"
        if highest_race == season_final_race
        else f"Update Race {highest_race}"
    )
...
def _build_footer(doc: Document, year: int, include_page_numbers: bool = False) -> None:
    ...
    center_run = center_p.add_run(f"WRRL Admin Suite v{__version__}")
