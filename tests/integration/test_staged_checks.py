...
def test_run_checks_stage4_generates_results_without_baseline_compare(monkeypatch, tmp_path):
    ...
    success, results = staged.run_checks(args)
    assert success is True
    stage4 = next(r for r in results if r.stage == 4)
    assert stage4.status == "passed"
    assert "fingerprint" in stage4.details


def test_run_checks_stage1_schema_failures_are_non_blocking(monkeypatch, tmp_path):
    data_root = tmp_path / "data"
    input_dir = data_root / "1999" / "inputs"
    output_dir = data_root / "1999" / "outputs"
    raw_dir = input_dir / "raw_data"
    raw_dir.mkdir(parents=True, exist_ok=True)
    ...
    monkeypatch.setattr(
        staged,
        "validate_race_schema",
        lambda df, _path: (_ for _ in ()).throw(RuntimeError("schema failure")),
    )
    monkeypatch.setattr(
        staged,
        "analyse_season",
    ...
    assert success is True
    stage1 = next(r for r in results if r.stage == 1)
    assert stage1.status == "passed_with_warnings"
    assert stage1.details["errors"]
